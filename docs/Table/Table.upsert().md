...
Similar to _update()_ but with upsert behavior. The difference between _upsert()_ and _update()_ is that _upsert()_ will create a new object if the key is not found, while _update()_ will not change anything.
...
Note: Be careful with nested object values. If you have an `address` field that includes `city`, `state`, and `zipcode`, then `db.friends.upsert(2, {address: {zipcode: 12345}})` will replace the entire `address` object with `{zipcode: 12345}`.
